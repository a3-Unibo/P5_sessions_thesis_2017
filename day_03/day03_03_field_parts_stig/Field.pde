class Field {
  ...
  void writeScalar(PVector p, float intensity) {
    int i = int(constrain(p.x/resX, 0, cols-1));
    int j = int(constrain(p.y/resY, 0, rows-1));
    scalar[i][j] += intensity;
    scalar[i][j] = constrain(scalar[i][j], 0, 1);
  }

  void evaporate(float ef) {
    for (int i=0; i< cols; i++) {
      for (int j=0; j<rows; j++) {
        scalar[i][j]*=ef;
        if (scalar[i][j]<0.0001) scalar[i][j]=0;
      }
    }
  }

  void evapLin(float ef) {
    for (int i=0; i< cols; i++) {
      for (int j=0; j<rows; j++) {
        scalar[i][j]-=ef;
        if (scalar[i][j]<0.0001) scalar[i][j]=0;
